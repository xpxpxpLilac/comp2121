...
RESET:
	...
	out PORTC, temp ; Enable pull-up resistors on PORTF
	clr temp		;pb0,pb1 connect to portL
	out DDRC, temp ; PORTF is all inputs
...
main:
	...
	ldi temp, 0xff   ; this value and the operation mode determine the PWM duty cycle sts OCR5AL, temp 
	sts OCR3BL,temp
	clr temp 
	sts OCR3BH, temp 
...
; COM5A1=1: make OC5A override the normal port functionality of the I/O pin PL3 
	...
	switch0:
	sbic PINC, 0 ; Skip the next instruction if PB0 is pushed
	rjmp switch1 ; If not pushed, check the other switch
	rcall sleep_350ms   
	rcall inc_20rps
	switch1:
	sbic PINC, 1 ; Skip the next instruction if PB1 is pushed
